...
import { Text, Alert, Pressable, StyleSheet, Dimensions, Image, View, ScrollView } from 'react-native';
...
export const YearChartCard = ({ navigation }: any) => {
  ...
  const generateYearData = () => {
    const years = Array.from(new Array(131), (val, index) => ({
      key: index,
      label: `${2015 + index}`,
    }));
    return years;
  };
  const years = generateYearData();
  ...
  return (
    <ScrollView>
      ...
      <ModalSelector
        data={years}
        ref={ref}
        selectedKey={years.find(year => year.label === selectedYear)?.key} // Ensure the selector centers on 2024
        onChange={(option) => {
          setSelectedYear(option.label);
          getMonthChartData(option.label);
        }}
        optionContainerStyle={{ height: windowWidth }}
        customSelector={
      ...
      {/* Pie Chart */}
      <PieChartDiv type={'Year'} pieDataB={pieDataB} pieData={pieData} />
...
const hiddenPickerStyles = StyleSheet.create({
  inputIOS: {
    height: 0,
    opacity: 0,
  },
